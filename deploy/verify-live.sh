#!/usr/bin/env bash
#
# Post-cutover verification (plan §6.4.5): curls every URL in the route
# contract (deploy/routes.php — every scan §2 URL, plus the frozen legacy
# redirects/410s) against a LIVE base URL and asserts the status plan §5.1.6
# requires. Run this after uploading to the Hostinger staging subdomain, and
# again after the DNS cutover to the production domain.
#
#     ./deploy/verify-live.sh https://staging.contador.com.py
#     ./deploy/verify-live.sh https://contador.com.py
#
...
BASE="${1:-}"
if [ -z "$BASE" ]; then
  echo "usage: $0 <base-url>   e.g. $0 https://staging.contador.com.py" >&2
  exit 2
fi
BASE="${BASE%/}"
