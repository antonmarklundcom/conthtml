...
- 2026-09-04 — B4 measurements: Lighthouse mobile against the **deploy zip** (not the raw repo — see KNOWN-ISSUES for why that's the representative number) on `php -S`: `/` **95 / 100 / 100** (perf/a11y/SEO, target ≥95/100/100), `/marangatu/` **98/100/100**, the aguinaldo article **98/100/100**. `./verify.sh` green on the repo and on the unzipped deploy artifact both before and after every change in this phase. Screenshots (1440/390) of `/`, `/servicios/`, `/precios/`, `/ips/`, `/marangatu/`, `/privacidad/` and two blog articles in `docs/screenshots/b4/`.
- 2026-09-04 — B4 open input, unchanged since §7: VenderCRM, Hostinger hosting/DNS access, and GA4/Ads ids are all still pending — every one of this phase's launch mechanics (`deploy/verify-live.sh`, the gtag.js snippet, the CRM forward) is built and tested in degraded/local mode but could not be exercised against a real live URL or real credentials from this session. `docs/launch-checklist.md` is the numbered list of exactly what Anton does next; per the phase prompt, **no further phase is spawned** after this one.
