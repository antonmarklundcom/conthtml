...
- Once B3 ships the calculadora de aguinaldo and the calculadora de liquidación de salario, add an
  inline link from `/blog/como-se-calcula-el-aguinaldo-en-paraguay/` to the aguinaldo calculator (the
  article currently links only to `/ips/`, the closest service page, since the tool didn't exist yet).
