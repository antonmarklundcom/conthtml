 ...
 * the 410s, the ?page_id=3 redirect, the wp-sitemap redirect, sitemap.xml,
 * trailing-slash enforcement, the denied directories, and the 404 document.
 *
 * Directory resolution is generic — any <dir>/index.php is served — so B2's
 * /blog/<slug>/ and B3's /herramientas/<slug>/ work here with no change.
 *
...
// --- gone: theme-demo and default-WordPress URLs (scan §2) ------------------
$gone = ['/single-service/', '/hello-world/', '/category/uncategorized/'];
if (in_array(rtrim($path, '/') . '/', $gone, true)) {
    $halt(410, '<h1>410 Gone</h1>');
    return true;
}

// --- 301s -------------------------------------------------------------------
if ($path === '/' && $query === 'page_id=3') {
    http_response_code(301);
    header('Location: /privacidad/');
    return true;
}
